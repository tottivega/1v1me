...
describe('numberguess — integration', () => {
  ...
  it('closer guess wins', () => {
    const room = makeRoom()
    room.match = makeMatch('p1', 'p2')
    numberguess.start(room)

    const state = room.match.minigameState as { secret: number }
    // p2 is closer regardless of the actual secret
    numberguess.handleInput(room, 'p1', { type: 'GUESS', value: Math.max(1, state.secret - 20) })
    numberguess.handleInput(room, 'p2', { type: 'GUESS', value: Math.max(1, state.secret - 1) })
  ...
  it('does not broadcast the secret during the guessing phase', async () => {
