...
// Build a 5-game queue that avoids consecutive same-category games.
...
export function shuffleQueue(): MinigameId[] {
  const allIds = Object.keys(MINIGAME_CONFIGS) as MinigameId[]
  ...
  while (queue.length < 5) {
    const lastCat = queue.length > 0
      ? MINIGAME_CONFIGS[queue[queue.length - 1]].category
      : null
    ...
    // Refill pool when exhausted (best-of-5 can exceed unique game count)
    if (pool.length === 0 && queue.length < 5) {
      const refill = [...allIds]
      fisherYates(refill)
      pool.push(...refill)
    }
