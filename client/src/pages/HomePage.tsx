...
const GAMES = Object.entries(MINIGAME_CONFIGS) as [string, typeof MINIGAME_CONFIGS[keyof typeof MINIGAME_CONFIGS]][]

function getSavedNickname() {
  try { return localStorage.getItem('nickname') ?? '' } catch { return '' }
}
function saveNickname(name: string) {
  try { localStorage.setItem('nickname', name) } catch {}
}
...
export default function HomePage() {
  ...
  function handleJoin() {
    ...
    if (!code) { setShake(true); setTimeout(() => setShake(false), 400); return }
    playReady()
    commit(nickname.trim())
    navigate(`/room/${code}`)
  }
  ...
  return (
    <div className="page" style={{ gap: 32 }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <div className="title" style={{ fontSize: 96 }}>1v1 ME</div>
      ...
      {/* Join / Create card */}
      <div className="card anim-pop" style={{ width: '100%', maxWidth: 420, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {mode === 'pick' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <button className="btn btn-orange btn-lg" style={{ width: '100%' }} onClick={handleCreate}>
              ⚔️ Create Room
            </button>
            <button className="btn btn-white" style={{ width: '100%' }} onClick={() => { playClick(); setMode('join') }}>
              🔗 Join with code
            </button>
          </div>
        )}
        ...
        {mode === 'join' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <button className="btn btn-white btn-sm" onClick={() => { playClick(); setMode('pick') }}>
              ← Back
            </button>
          </div>
        )}
      ...
      {/* Decorative */}
      <div style={{ position: 'fixed', bottom: 20, right: 24, fontSize: 48, opacity: 0.15, pointerEvents: 'none', userSelect: 'none', transform: 'rotate(-12deg)' }}>🕹️</div>
      <div style={{ position: 'fixed', top: 24, left: 24, fontSize: 36, opacity: 0.13, pointerEvents: 'none', userSelect: 'none', transform: 'rotate(8deg)' }}>⭐</div>
    </div>
  )
}
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
          <span style={{
            background: catColor, color: isLightCat ? 'var(--black)' : 'var(--white)',
      ...
      <div style={{ fontFamily: 'var(--font-title)', fontSize: 17, color: 'var(--black)', lineHeight: 1.1 }}>
        {cfg.label}
      </div>
