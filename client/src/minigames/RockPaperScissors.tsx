...
const INITIAL_MOCK: MockState = {
  throwNum: 1, phase: 'picking', myPick: null,
  revealPicks: {}, throwWinnerId: null,
  scores: {}, history: [], finalWinnerId: null,
}
...
export default function RockPaperScissors() {
  ...
  function resolveMockThrow(prev: MockState, myChoice: Choice, oppChoice: Choice): MockState {
    ...
    const base: MockState = {
      ...prev, phase: 'reveal', myPick: myChoice,
      revealPicks, throwWinnerId: winnerId,
      scores: newScores, history: newHistory,
    }

    if (matchWinner || allDone) {
      const s1 = newScores[myPlayerId] ?? 0
      const s2 = newScores[oppId] ?? 0
      const finalWinnerId = s1 > s2 ? myPlayerId : s2 > s1 ? oppId : (Math.random() < 0.5 ? myPlayerId : oppId)
  ...
  const iHavePicked = isLive
    ? (serverState?.phase === 'picking' && (serverState as ServerStatePicking).submitted.includes(myPlayerId))
    : mock.myPick !== null

  const revealPicks: Record<string, Choice> | null =
    isLive ? (liveReveal?.picks ?? null) : (mock.phase === 'reveal' ? mock.revealPicks : null)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, flex: 1, padding: 24 }}>
      <div style={{ fontFamily: 'var(--font-title)', fontSize: 32, color: 'var(--red)', textAlign: 'center' }}>
      ...
      <div style={{ fontFamily: 'var(--font-title)', fontSize: 13, letterSpacing: 2, opacity: 0.45, textTransform: 'uppercase' }}>
      ...
      {finalWinnerId && (
        <div className="anim-bounce" style={{ textAlign: 'center' }}>
          <div style={{
            fontFamily: 'var(--font-title)', fontSize: 52,
      ...
      {!finalWinnerId && phase === 'picking' && (
        <>
          <div style={{ fontFamily: 'var(--font-title)', fontSize: 16, opacity: 0.5, letterSpacing: 1 }}>
            {iHavePicked ? `Waiting for ${oppName}…` : 'MAKE YOUR PICK'}
          </div>
          <div style={{ display: 'flex', gap: 16 }}>
            {CHOICES.map(({ id, emoji, label }) => {
              const isPicked = isLive
                ? myLivePickRef.current === id
                : mock.myPick === id
              return (
                <button
                  key={id}
                  ...
                  style={{
                    ...
                    borderRadius: 16, boxShadow: isPicked ? '2px 2px 0 var(--black)' : '4px 4px 0 var(--black)',
                    transform: isPicked ? 'translate(2px, 2px)' : 'none',
                    cursor: iHavePicked ? 'default' : 'pointer',
                    opacity: iHavePicked && !isPicked ? 0.35 : 1,
                    transition: 'all 0.1s', fontSize: 40, padding: 0,
                  }}
                >
                  {emoji}
                  <span style={{
      ...
      {!finalWinnerId && phase === 'reveal' && revealPicks && (
        <div className="anim-pop" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
          <div style={{ display: 'flex', gap: 28, alignItems: 'center' }}>
            <PickDisplay choice={revealPicks[myPlayerId]} label="You"   isWinner={revealWinner === myPlayerId} />
            <div style={{ fontFamily: 'var(--font-title)', fontSize: 20, opacity: 0.4 }}>VS</div>
            <PickDisplay choice={revealPicks[oppId]}      label={oppName} isWinner={revealWinner === oppId} />
          </div>
          <div style={{
            fontFamily: 'var(--font-title)', fontSize: 20,
            color: revealWinner === myPlayerId ? 'var(--green)' : revealWinner === null ? 'var(--orange)' : 'var(--red)',
          }}>
            {revealWinner === myPlayerId ? '🏆 You take this throw!' : revealWinner === null ? '🤝 Tie!' : `${oppName} takes this throw!`}
          </div>
        </div>
      )}
          ...
          {history.map((record, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                fontFamily: 'var(--font-title)', fontSize: 14, color: 'var(--white)',
              }}>
                {tied ? '–' : won ? '✓' : '✗'}
              </div>
            )
...
function ThrowScore({ label, score, color }: { label: string; score: number; color: string }) {
  return (
    <div style={{ textAlign: 'center' }}>
      <div style={{ fontFamily: 'var(--font-title)', fontSize: 52, color, lineHeight: 1, textShadow: '3px 3px 0 var(--black)' }}>
        {score}
      </div>
      <div style={{ fontSize: 12, fontWeight: 700, opacity: 0.55, textTransform: 'uppercase', letterSpacing: 1 }}>{label}</div>
    </div>
  )
}

function PickDisplay({ choice, label, isWinner }: { choice: Choice | undefined; label: string; isWinner: boolean }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontFamily: 'var(--font-title)', fontSize: 12, opacity: 0.55 }}>{label.toUpperCase()}</span>
    </div>
  )
}
