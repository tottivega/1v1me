...
export default function CoinFlip() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 32, flex: 1, padding: 32 }}>
      <div style={{ fontFamily: 'var(--font-title)', fontSize: 36, color: 'var(--yellow)', textAlign: 'center' }}>
        COIN FLIP 🪙
      </div>
      ...
      <div style={{
        ...
        border: '5px solid var(--black)', boxShadow: '6px 6px 0px var(--black)',
        background: phase === 'flipping' ? 'var(--yellow)' : iWon ? 'var(--green)' : 'var(--red)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: 72,
        animation: phase === 'flipping' ? 'coin-flip-3d 0.65s linear infinite' : 'bounce-in 0.4s ease both',
      ...
      {phase === 'result' && (
        <div className="anim-bounce" style={{ textAlign: 'center' }}>
          <div style={{
            ...
            WebkitTextStroke: '2px var(--black)', textShadow: '3px 3px 0px var(--black)',
          }}>
            {iWon ? 'YOU WIN! 🎉' : 'YOU LOSE 😤'}
          </div>
          <div className="subtitle" style={{ opacity: 0.6, marginTop: 8 }}>
            {iWon ? 'The coin smiled upon you today.' : `${opponent?.nickname ?? 'Opponent'} got lucky this time.`}
          </div>
        </div>
      )}

      {phase === 'flipping' && (
        <div className="subtitle anim-pulse" style={{ opacity: 0.7 }}>Flipping…</div>
      )}
