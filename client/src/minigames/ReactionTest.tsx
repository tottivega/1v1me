...
export default function ReactionTest() {
  ...
  const [localClickedAt, setLocalClickedAt] = useState<number | null>(null)
  ...
  if (isLive) {
    ...
    const bgColor =
      iAmPenalized  ? '#880000' :
      phase === 'ready' ? 'var(--green)' :
      showResult ? '#222222' :
      '#cc2222'

    return (
      <div onClick={handleLiveClick} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, background: bgColor, transition: 'background 0.15s', cursor: showResult ? 'default' : 'pointer', userSelect: 'none', borderTop: 'var(--border)' }}>
        {phase === 'waiting' && !iAmPenalized && (
          <>
            <div style={{ fontFamily: 'var(--font-title)', fontSize: 48, color: 'var(--white)', textShadow: '3px 3px 0 rgba(0,0,0,0.4)' }}>⚠️ WAIT…</div>
            <div style={{ color: 'rgba(255,255,255,0.7)', fontWeight: 700, fontSize: 18 }}>Click when the screen turns green!</div>
            {hasSentReaction.current && <div className="badge badge-red">⚡ Input sent — waiting…</div>}
          </>
        )}
        {iAmPenalized && (
          <div className="anim-shake" style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-title)', fontSize: 52, color: 'var(--yellow)', textShadow: '3px 3px 0 rgba(0,0,0,0.4)' }}>TOO EARLY! 💀</div>
            <div style={{ color: 'rgba(255,255,255,0.7)', fontWeight: 700, marginTop: 12, fontSize: 18 }}>You clicked too soon. Opponent wins.</div>
          </div>
        )}
        {phase === 'ready' && !hasSentReaction.current && (
          <div className="anim-pop">
            <div style={{ fontFamily: 'var(--font-title)', fontSize: 72, color: 'var(--white)', textShadow: '4px 4px 0 rgba(0,0,0,0.4)' }}>🟢 CLICK NOW!</div>
          </div>
        )}
        {phase === 'ready' && hasSentReaction.current && (
          <div style={{ fontFamily: 'var(--font-title)', fontSize: 52, color: 'var(--white)' }}>⚡ Reacted!</div>
        )}
        {showResult && (
          <div className="anim-bounce" style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-title)', fontSize: 52, color: iWon ? 'var(--yellow)' : 'var(--red)', textShadow: '3px 3px 0 rgba(0,0,0,0.4)' }}>
            ...
            <div style={{ display: 'flex', gap: 32, marginTop: 24, justifyContent: 'center' }}>
              ...
              {oppReactionMs !== null && <ReactionTime label={opponent?.nickname ?? 'Opponent'} ms={oppReactionMs} />}
            </div>
          </div>
        )}
      </div>
  ...
  // Mock mode
  const mockIWon = mockMyTime !== null && mockOppTime !== null
    ? mockMyTime < mockOppTime
    : mockPhase === 'too-early' ? false : mockMyTime !== null

  const mockBg =
    mockPhase === 'waiting'   ? '#cc2222' :
    mockPhase === 'too-early' ? '#880000' :
    mockPhase === 'ready'     ? 'var(--green)' :
    '#222222'

  return (
    <div onClick={handleMockClick} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, background: mockBg, transition: 'background 0.15s', cursor: mockPhase === 'result' ? 'default' : 'pointer', userSelect: 'none', borderTop: 'var(--border)' }}>
      {mockPhase === 'waiting' && (
        <>
          <div style={{ fontFamily: 'var(--font-title)', fontSize: 48, color: 'var(--white)', textShadow: '3px 3px 0 rgba(0,0,0,0.4)' }}>⚠️ WAIT…</div>
          <div style={{ color: 'rgba(255,255,255,0.7)', fontWeight: 700, fontSize: 18 }}>Click when the screen turns green!</div>
      ...
      {mockPhase === 'too-early' && (
        <div className="anim-shake" style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: 'var(--font-title)', fontSize: 52, color: 'var(--yellow)', textShadow: '3px 3px 0 rgba(0,0,0,0.4)' }}>TOO EARLY! 💀</div>
          <div style={{ color: 'rgba(255,255,255,0.7)', fontWeight: 700, marginTop: 12, fontSize: 18 }}>You're disqualified. Opponent wins.</div>
        </div>
      )}
      {mockPhase === 'ready' && (
        <div className="anim-pop">
      ...
      {mockPhase === 'clicked' && (
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: 'var(--font-title)', fontSize: 52, color: 'var(--white)' }}>⚡ {mockMyTime}ms</div>
          <div style={{ color: 'rgba(255,255,255,0.7)', fontWeight: 700, fontSize: 18, marginTop: 8 }}>Waiting for result…</div>
        </div>
      )}
      {mockPhase === 'result' && (
        <div className="anim-bounce" style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: 'var(--font-title)', fontSize: 52, color: mockIWon ? 'var(--yellow)' : 'var(--red)', textShadow: '3px 3px 0 rgba(0,0,0,0.4)' }}>
            {mockIWon ? '🏆 YOU WIN!' : '💀 YOU LOSE'}
          </div>
          <div style={{ display: 'flex', gap: 32, marginTop: 24, justifyContent: 'center' }}>
            {mockMyTime  && <ReactionTime label="You" ms={mockMyTime} />}
            {mockOppTime && <ReactionTime label={opponent?.nickname ?? 'Opponent'} ms={mockOppTime} />}
          </div>
        </div>
      )}
    </div>
