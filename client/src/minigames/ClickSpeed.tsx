...
export default function ClickSpeed() {
  ...
  const myClicks  = isLive ? Math.max(localMyClicks, serverState?.clicks[myPlayerId] ?? 0) : localMyClicks
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 32, flex: 1, padding: 32 }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div className="subtitle" style={{ opacity: 0.6, fontSize: 15 }}>Click as fast as you can!</div>
      </div>

      <div style={{ display: 'flex', gap: 24, width: '100%', maxWidth: 480, alignItems: 'flex-end' }}>
      ...
      <button
        ...
        onTouchStart={e => { e.preventDefault(); const t = e.touches[0]; handleClick(t.clientX, t.clientY) }}
        style={{
          position: 'relative', overflow: 'hidden',
          width: 180, height: 180, borderRadius: '50%', fontSize: 52,
          border: '4px solid var(--black)', boxShadow: '6px 6px 0px var(--black)',
      ...
      <div className="label" style={{ opacity: 0.5 }}>Smash that button!</div>
    </div>
  )
}
...
function ClickBar({ label, clicks, color }: { label: string; clicks: number; color: string }) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <span style={{ fontFamily: 'var(--font-title)', fontSize: 48, color, lineHeight: 1 }}>{clicks}</span>
      <div style={{ width: '100%', height: 12, background: '#e0e0e0', border: '2px solid var(--black)', borderRadius: 99, overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${Math.min(100, (clicks / 80) * 100)}%`, background: color, borderRadius: 99, transition: 'width 0.1s' }} />
      </div>
